...
export class StrykerInitializer {
    ...
    async initiateCustom(configWriter) {
        ...
        const configFileName = await configWriter.write(selectedTestRunner, buildCommand, selectedReporters, selectedPackageManager, await this.fetchAdditionalConfig(npmDependencies), isJsonSelected);
    ...
    installNpmDependencies(dependencies, selectedOption) {
        ...
        const cmd = selectedOption.name === "npm" /* PackageManager.Npm */ ? `npm i --save-dev ${dependencyArg}` : `yarn add ${dependencyArg} --dev`;
        this.out(cmd);
        try {
...
StrykerInitializer.inject = tokens(commonTokens.logger, initializerTokens.out, initializerTokens.npmClient, initializerTokens.strykerPresets, initializerTokens.configWriter, initializerTokens.gitignoreWriter, initializerTokens.inquirer);
