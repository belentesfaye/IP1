...
export class MutationTestReportHelper {
    constructor(reporter, options, project, log, dryRunResult, requireFromCwd) {
    ...
    async toFileResults(results, remapTestIds) {
        ...
        return results.reduce((acc, mutantResult) => {
            var _a;
            const fileResult = (_a = acc[mutantResult.fileName]) !== null && _a !== void 0 ? _a : (acc[mutantResult.fileName] = fileResultsByName.get(mutantResult.fileName));
            fileResult.mutants.push(this.toMutantResult(mutantResult, remapTestIds));
            return acc;
        }, {});
    }
    async toTestFiles(remapTestId) {
        const testResultsByName = new Map(await Promise.all([...new Set(this.dryRunResult.tests.map(({ fileName }) => fileName))].map(async (fileName) => [fileName !== null && fileName !== void 0 ? fileName : '', await this.toTestFile(fileName)])));
        return this.dryRunResult.tests.reduce((acc, testResult) => {
            var _a, _b;
            const test = this.toTestDefinition(testResult, remapTestId);
            const fileName = (_a = testResult.fileName) !== null && _a !== void 0 ? _a : ''; // by default we accumulate tests under the '' key
            const testFile = (_b = acc[fileName]) !== null && _b !== void 0 ? _b : (acc[fileName] = testResultsByName.get(fileName));
...
MutationTestReportHelper.inject = tokens(coreTokens.reporter, commonTokens.options, coreTokens.project, commonTokens.logger, coreTokens.dryRunResult, coreTokens.requireFromCwd);
